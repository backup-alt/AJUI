...
export class RequestsPage implements OnInit {
  ...
  async loadAllRequests(): Promise<void> {
    ...
    try {
      // Load materials
      try {
        const matRes = await firstValueFrom(this.supervisor.getMaterials({ limit: 200 }).pipe(timeout(15000)));
      ...
      try {
        const expRes = await firstValueFrom(this.supervisor.getExpenses({ type: 'site', limit: 200 }).pipe(timeout(15000)));
