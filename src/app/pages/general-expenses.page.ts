...
import { mapClient, mapProject, mapSite, mapVendor, mapSupervisor, mapMaterial, mapLabour, mapExpense, mapGeneralExpense, mapPayment, mapSubcontractor, mapInventory } from "../core/mappers";
...
const dashboardModules: ModuleConfig[] = [
  ...
  },
  {
    ...
    label: "Clients",
    title: "All Clients",
    description: "Every client with contact, project count, value, received amount, and pending balance.",
    columns: [
  ...
  },
  {
    ...
    description: "Every staff attendance row with labour types, staff count, shift, overtime, and fine.",
    columns: [
      { key: "client", label: "Client" },
      { key: "project", label: "Project" },
  ...
  {
    key: "expenses",
    label: "Supervisor Expenses",
    title: "Supervisor Expense Details",
    description: "Project-linked supervisor expense ledger.",
    columns: [
      { key: "client", label: "Client" },
      { key: "project", label: "Project" },
  ...
  },
  {
    ...
    description: "Office expenses such as admin purchases, courier, printing, and overhead payments.",
    columns: [
      { key: "expenseDate", label: "Expense Date" },
      { key: "description", label: "Description" },
      { key: "amount", label: "Amount" },
  ...
  },
  {
    ...
    description: "Aggregated material stock by type across all projects.",
    columns: [
...
@Component({
    ...
    <ion-split-pane contentId="main-content" when="lg">
      ...
      <div class="ion-page" id="main-content">
        <agb-enterprise-header
          [title]="isGeneralExpensesPage ? 'General Expenses' : 'Dashboard'"
          [eyebrow]="isGeneralExpensesPage ? 'Office expense ledger' : 'Universal Records · Backend source of truth'"
          metaLabel=""
        ...
        <ion-content class="erp-page">
          <main class="workspace-shell" [class.table-view-expanded]="tableViewExpanded()">
            <section class="dashboard-command-strip dashboard-command-center" *ngIf="!isGeneralExpensesPage && !tableViewExpanded()">
            ...
            <section class="operations-workbench universal-workbench" [class.table-expanded]="tableViewExpanded()">
              <nav class="operations-tabs" aria-label="Universal dashboard modules" *ngIf="!isGeneralExpensesPage && !tableViewExpanded()">
                <button
              ...
              <section class="general-expense-total-strip" *ngIf="!tableViewExpanded() && activeModule() === 'generalExpenses'">
                <div class="general-expense-total-primary">
                  <span>{{ hasDateFilter() ? 'Expense for selected period' : 'Total Expense' }}</span>
                  <strong>{{ formatMoney(generalExpenseFilteredTotal()) }}</strong>
                </div>
                <div>
                  <span>Period</span>
                  <strong>{{ generalExpensePeriodLabel() }}</strong>
                </div>
                <div>
                  <span>Matching records</span>
                  <strong>{{ generalExpenseFilteredCount() }}</strong>
                </div>
              </section>
              ...
              <ng-container *ngIf="activeModule() !== 'inventory'">
                ...
                <span *ngIf="activeModule() === 'expenses'">Balances grouped by project</span>
            ...
            <section class="erp-dialog inventory-breakdown-dialog" role="dialog" aria-modal="true" aria-labelledby="inv-breakdown-title">
              <div class="dialog-head">
                <div>
                  ...
                  <p>Full project add/issue history, newest first</p>
                </div>
              ...
              <div class="inventory-breakdown-table">
                ...
                @for (row of inventoryBreakdownRows(); track row.id) {
                  <div class="breakdown-table-row">
                    <span>{{ row.projectName || row.project || 'Project' }}</span>
  ...
  styles: [`
    ...
    .general-expense-total-strip {
      display: grid;
      grid-template-columns: minmax(240px, 1.35fr) repeat(2, minmax(180px, 0.75fr));
      gap: 12px;
      padding: 14px 16px;
    ...
    .general-expense-total-strip > div {
      display: grid;
      gap: 5px;
      padding: 13px 15px;
    ...
    .general-expense-total-strip span { color: #64748b; font-size: 11px; font-weight: 700; }
    .general-expense-total-strip strong { color: #172033; font-size: 15px; }
    .general-expense-total-strip .general-expense-total-primary {
      border-color: #b9cdf3;
      background: #eef4ff;
    }
...
export class GeneralExpensesPage implements OnInit {
  ...
  readonly isGeneralExpensesPage = this.router.url.split("?")[0] === "/general-expenses";
  readonly modules = dashboardModules.filter((module) => module.key !== "generalExpenses");
  readonly formatMoney = formatMoney;
  readonly statusClass = statusClass;
  readonly activeModule = signal<DashboardModule>(this.isGeneralExpensesPage ? "generalExpenses" : "clients");
  ...
  readonly inlineVendorSaving = signal(false);
  readonly uploadingGeneralExpenseBills = signal<string[]>([]);
  ...
  readonly generalExpenseFilteredTotal = computed(() =>
    this.activeModule() === "generalExpenses"
      ? this.visibleRows().reduce((sum, row) => sum + Math.abs(this.moneyNumber(row["amount"])), 0)
      : 0,
  );
  readonly generalExpenseFilteredCount = computed(() =>
    this.activeModule() === "generalExpenses" ? this.visibleRows().length : 0,
  );
  ...
  readonly hasMoreRows = computed(() => {
    ...
    if (this.isGeneralExpensesPage && module === "generalExpenses") return localMore;
    const serverMore = this.hydration.hasMorePages(module);
  ...
  readonly totalCount = computed(() => {
    ...
    if (this.isGeneralExpensesPage && module === "generalExpenses") {
      return this.data.generalExpenses().length;
    }
    return this.hydration.getTotalCount(module);
  });
  ...
  private async loadAllGeneralExpenses(): Promise<void> {
    try {
      const result = await firstValueFrom(this.api.listAllGeneralExpenses());
      this.data.generalExpenses.set((result.items || []).map(mapGeneralExpense));
    } catch (error) {
      console.warn("[generalExpenses] full list load failed; falling back to paginated data", error);
      await this.hydration.loadModule("generalExpenses");
    }
  }
  ...
  async loadMoreRows(): Promise<void> {
    ...
    if (localRows.length > this.displayLimit()) {
      ...
      if (this.hydration.hasMorePages(module)) {
        const loaded = await this.hydration.loadNextPage(module as PageModule);
  ...
  private async executeServerSearch(query: string): Promise<void> {
    ...
    try {
      const apiMap: Record<string, (opts: any) => any> = {
        ...
        generalExpenses: (opts: any) => firstValueFrom(this.api.listGeneralExpenses(opts)),
  ...
  switchModule(module: DashboardModule) {
    ...
    } else if (module === "generalExpenses") {
  ...
  isGeneralExpenseBillUploading(row: TableRow): boolean {
    return this.uploadingGeneralExpenseBills().includes(this.rowKey(row));
  }

  async uploadGeneralExpenseBill(row: TableRow, event: Event): Promise<void> {
    event.stopPropagation();
    ...
    if (!/^[a-f\d]{24}$/i.test(mongoId)) {
      await this.presentToast("General expense record could not be found.", "danger");
      input.value = "";
      return;
    }

    const uploadKey = this.rowKey(row);
    this.uploadingGeneralExpenseBills.update((keys) => [...new Set([...keys, uploadKey])]);
    try {
      const data = await this.fileAsBase64(file);
      const response = await firstValueFrom(this.api.uploadGeneralExpenseReceipt(mongoId, {
        data,
        mimeType: file.type,
        fileName: file.name,
      }));
      ...
      this.uploadingGeneralExpenseBills.update((keys) => keys.filter((key) => key !== uploadKey));
      input.value = "";
    }
  }
  ...
  private fileAsBase64(file: File): Promise<string> {
    return new Promise((resolve, reject) => {
      ...
      reader.onerror = () => reject(new Error("Could not read the selected bill."));
  ...
  generalExpensePeriodLabel(): string {
    const range = this.dateRange();
    if (!range.start && !range.end) return "All dates";
    const start = range.start ? this.dateDisplay(range.start) : "Beginning";
    const end = range.end ? this.dateDisplay(range.end) : "Today";
    return `${start} – ${end}`;
  }
  ...
  async createInlineVendor(value: VendorFormValue) {
    ...
    this.inlineVendorSaving.set(true);
    try {
      await new Promise<void>((resolve, reject) => {
        this.api.createVendor(payload).subscribe({ next: () => resolve(), error: reject });
      });
      this.showVendorDialog.set(false);
      this.editingInlineVendor.set(null);
      this.data.addVendor({
      ...
      window.alert("Could not create the vendor. Please try again.");
      // Keep the dialog open so the user can correct and retry.
    } finally {
      this.inlineVendorSaving.set(false);
    }
  }

  async updateInlineVendor(value: VendorFormValue) {
    ...
    try {
      await new Promise<void>((resolve, reject) => {
        this.api.patchVendor(inline.id, payload).subscribe({ next: () => resolve(), error: reject });
      });
      this.showVendorDialog.set(false);
      this.editingInlineVendor.set(null);
      ...
      window.alert("Could not save the vendor changes. Please try again.");
      // Keep the dialog open so the user can correct and retry.
    } finally {
      this.inlineVendorSaving.set(false);
    }
  ...
  recordFormColumns(): FieldSchema[] {
    ...
    return this.columnsForActive().filter((column) => {
      ...
      if (this.activeModule() === "generalExpenses" && column.key === "reference") return false;
  ...
  updateDraftField(key: string, value: string) {
    this.draftRow.update((row) => {
      ...
      next["materialName"] = next["materialName"] || next["description"] || "Material purchase";
          next["unit"] = unit;
      ...
      if (this.activeModule() === "materials" && key === "materialName") {
  ...
  private preferredUnitForMaterialName(name: string): string {
    ...
    if (!candidates.length) {
      for (const row of this.rowsFor("materials")) {
  ...
  async saveRecord(event: Event) {
    ...
    try {
      await this.performSaveRecord(module, row);
    } finally {
      this.recordSaving.set(false);
    }
  }

  private async performSaveRecord(module: SharedModuleKey, row: TableRow): Promise<void> {
    if (module === "clients") {
      ...
      if (!description || !Number.isFinite(amount) || amount <= 0) {
        await this.presentToast("Description and an amount greater than zero are required.", "warning");
        return;
      }
      try {
        const response = await firstValueFrom(this.api.createGeneralExpense({
          origin: "manual",
          category: "Office Expense",
          amount,
          date: String(row["expenseDate"] || new Date().toISOString().slice(0, 10)),
          description,
        ...
        const saved = mapGeneralExpense(response?.expense || response);
        this.data.generalExpenses.update((items) => [saved, ...items]);
        this.recordDialogOpen.set(false);
      } catch (err: any) {
        await this.presentToast(err?.error?.message || err?.message || "Could not save the general expense.", "danger");
      }
      return;
    } else {
  ...
  private async presentToast(message: string, color: "success" | "warning" | "danger" = "success") {
    try {
      ...
      console.warn("[UniversalDashboard] Failed to present toast:", err);
    }
  }
   ...
   * project and subcontractor by name and creates a real record.
   */
  ...
  private persistRowEditToBackend(module: string, row: TableRow, columnKey: string, value: string) {
    ...
    const apiCall = (() => {
      ...
      switch (module) {
        ...
        case "generalExpenses": return this.api.patchGeneralExpense(mongoId, backendPayload);
  ...
  private buildRows(): Record<DashboardModule, TableRow[]> {
    ...
    const generalExpenses = this.data.generalExpenses().map((row) => ({
      _id: row._id,
      id: row.id,
    ...
    return { materials, clients, labour, expenses, generalExpenses, payments, vendors, subcontractors, inventory };
  }
