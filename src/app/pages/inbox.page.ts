...
@Component({
  standalone: true,
  imports: [CommonModule, FormsModule, IonContent, IonSplitPane, EnterpriseSidebarComponent, SearchableSelectComponent],
  template: `<ion-split-pane contentId="main-content" when="lg"><agb-enterprise-sidebar /><div class="ion-page" id="main-content"><ion-content class="inbox-content"><main>
    ...
    <section><div class="section-head"><div><h2>Messages</h2><p>Private messages with the office.</p></div><b>Page {{ messagePage() }}</b></div>
      <div class="message-list">@for (message of messages(); track message._id) { <article><span class="avatar">{{ senderInitial(message.senderName) }}</span><div><header><strong>{{ message.senderName }}</strong><time>{{ message.createdAt | date:'medium' }}</time></header><p class="message">{{ message.text }}</p><div class="actions">@if (isAdmin()) { <button (click)="chooseRecipient(message.ownerId)">Reply privately</button> }@if (message.link) { <a class="request-link" [href]="message.link">Open requested record</a> }</div></div></article> } @empty { <div class="empty"><strong>No messages yet</strong><span>New private messages will appear here.</span></div> }</div>
      ...
      <div class="composer">@if (isAdmin()) { <label class="recipient">Send privately to<agb-searchable-select [value]="replyTo()" [options]="recipientOptions()" placeholder="Select employee" [contained]="true" (valueChange)="chooseRecipient($any($event))" /></label> }<label>{{ isAdmin() ? 'Message' : 'Message admin' }}<textarea [(ngModel)]="draft" maxlength="4000" rows="4" placeholder="Write a message…"></textarea></label>@if (requestLink()) { <p class="attached-link">The exact record link will be attached for Admin.</p> }<div><button (click)="cancel()">Clear</button><button class="primary" (click)="send()" [disabled]="busy() || !draft.trim() || (isAdmin() && !replyTo())">Send message</button></div></div>
    </section>
...
export class InboxPage {
  ...
  readonly recipientOptions = computed(() => this.recipients().map((employee) => ({
    label: `${employee.name} · ${String(employee.role || "employee").replace(/_/g, " ")}`,
  ...
  async refresh() {
    ...
    try {
      ...
      if (this.isAdmin()) jobs.push(this.fetchActivity(this.activityPage()), this.fetchRecipients());
  ...
  chooseRecipient(ownerId: string) { this.replyTo.set(String(ownerId || "")); }
  cancel() { this.replyTo.set(""); this.requestLink.set(""); this.draft = ""; }
  async send() {
    this.busy.set(true); this.error.set("");
    try { await firstValueFrom(this.api.saveInboxMessage({text: this.draft.trim(), ownerId: this.replyTo() || undefined, link: this.requestLink() || undefined})); this.cancel(); this.messagePage.set(1); await this.fetchMessages(1); }
