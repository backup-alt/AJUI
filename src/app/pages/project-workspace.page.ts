                ...
                <div class="table-actions">
                  ...
                  <button
                    ...
                    *ngIf="!tableViewExpanded() && activeSection() === 'expenses'"
                    ...
                    [attr.aria-label]="fundingSupervisorHasOpeningAmount() ? 'Add Cash' : 'Add Opening Cash'"
                  >
                    <ion-icon name="cash-outline"></ion-icon>
                    {{ fundingSupervisorHasOpeningAmount() ? 'Add Cash' : 'Add Opening Cash' }}
                  </button>
                                ...
                                <div class="material-bill-actions">
                                  @if (materialBillHistory(row).length > 1) {
                                    ...
                                    <button
                                      type="button"
                                      class="material-po-trigger"
                                      [attr.aria-expanded]="isBillMenuOpen(row)"
                                      (pointerdown)="$event.stopPropagation()"
                                      (click)="toggleBillMenu(row, $event)"
                                    >
                                      <span>View Bills</span>
                                      <small>{{ materialBillHistory(row).length }} Bills</small>
                                      <svg viewBox="0 0 20 20" aria-hidden="true" class="svg-icon"><path d="M5.5 7.5 10 12l4.5-4.5" /></svg>
                                    </button>
                                    <div class="material-po-panel" *ngIf="isBillMenuOpen(row)">
                                      <header>
                                        <strong>Uploaded bills</strong>
                                        <span>{{ materialBillHistory(row).length }} files</span>
                                      </header>
                                      <div class="material-po-list">
                                        @for (bill of materialBillHistory(row); track bill.url; let first = $first) {
                                          @if (isDataUrl(bill.url)) {
                                            <button type="button" class="material-po-option material-bill-option" (click)="openMaterialBill(bill.url, $event)">
                                              <span class="material-po-date">{{ formatMaterialPoDate(bill.date) }}</span>
                                              <strong>{{ bill.label }}</strong>
                                              <small *ngIf="first">Latest</small>
                                              <em class="material-po-qty">{{ bill.poNumber || 'No purchase order' }}</em>
                                            </button>
                                          } @else {
                                            <a class="material-po-option material-bill-option" [href]="bill.url" target="_blank" rel="noopener noreferrer" (click)="closeBillMenu($event)">
                                              <span class="material-po-date">{{ formatMaterialPoDate(bill.date) }}</span>
                                              <strong>{{ bill.label }}</strong>
                                              <small *ngIf="first">Latest</small>
                                              <em class="material-po-qty">{{ bill.poNumber || 'No purchase order' }}</em>
    ...
    <section class="form-overlay funding-overlay" *ngIf="fundingSupervisorId()" (click)="closeSupervisorFunding()">
      <div class="erp-dialog operations-dialog funding-dialog" (click)="$event.stopPropagation()">
        <div class="dialog-head">
          ...
          <button type="button" class="icon-button" (click)="closeSupervisorFunding()" aria-label="Close">
            <ion-icon name="close-outline"></ion-icon>
          </button>
        </div>
        <div class="erp-form funding-form">
          <label class="span-2" *ngIf="projectSites().length > 1">
            <span>Site</span>
          ...
          @if (fundingSuccess()) {
            <div class="funding-feedback funding-success span-2" role="status">{{ fundingSuccess() }}</div>
          }
        </div>
        <div class="dialog-actions">
          <button type="button" class="secondary-action" (click)="closeSupervisorFunding()" [disabled]="fundingSaving()">Close</button>
          <button type="button" class="primary-action" (click)="submitSupervisorFunding()" [disabled]="fundingSaving()" [attr.aria-busy]="fundingSaving() ? 'true' : null">
            @if (fundingSaving()) {
              <span class="agb-loading-spinner" aria-hidden="true"></span>
            }
...
export class ProjectWorkspacePage {
  ...
  closeTransientTableUi(event: PointerEvent) {
    ...
    if (!target.closest(".material-bill-history")) {
      this.openBillHistoryKey.set("");
    }
  }
  ...
  private consolidateMaterialRows(rows: TableRow[]): TableRow[] {
    ...
    return [...groups.entries()].map(([key, group]) => {
      ...
      return {
        ...
        __billHistoryJson: JSON.stringify(this.consolidatedMaterialBillHistory(group)),
      };
    });
  }
  ...
  private buildInitialRows(projectId: string): Record<ModuleKey, TableRow[]> {
    ...
    const expenses = this.data.expensesForProject(projectId).filter((row) => row.type === "Site Expense").map((row) => ({
        ...
        runningBalance: formatMoney(row.runningBalance ?? 0),
        site: row.site,
  ...
  private withExpenseBalances(rows: TableRow[]): TableRow[] {
    ...
    const computedRows = this.expenseChronologicalRows(rows).map((row) => {
      ...
      if (!balances.has(groupKey)) {
        balances.set(groupKey, this.expenseLedgerOpeningFor(row));
      }
  ...
  materialBillHistory(row: TableRow): MaterialBillHistoryEntry[] {
    try {
      const history = JSON.parse(String(row["__billHistoryJson"] || "[]"));
      if (Array.isArray(history)) {
        return history
          .map((entry: any, index: number) => ({
            url: String(entry?.url || "").trim(),
            label: String(entry?.label || `Bill ${index + 1}`).trim(),
            date: String(entry?.date || ""),
            poNumber: this.readablePurchaseOrderNumber(entry?.poNumber),
          }))
          .filter((entry: MaterialBillHistoryEntry) => Boolean(entry.url))
          .sort((a: MaterialBillHistoryEntry, b: MaterialBillHistoryEntry) => Date.parse(b.date || "") - Date.parse(a.date || ""));
      }
    } catch {
      // Fall through to the row's single bill for legacy rows.
    }
    return this.materialBillHistorySources(row);
  }

  isBillMenuOpen(row: TableRow): boolean {
    return this.openBillHistoryKey() === this.rowKey(row);
  }

  toggleBillMenu(row: TableRow, event?: Event): void {
    event?.preventDefault();
    event?.stopPropagation();
    const key = this.rowKey(row);
    this.openPoHistoryKey.set("");
    this.openBillHistoryKey.update((current) => current === key ? "" : key);
  }

  closeBillMenu(event?: Event): void {
    event?.stopPropagation();
    this.openBillHistoryKey.set("");
  }

  openMaterialBill(url: string, event?: Event): void {
    event?.preventDefault();
    event?.stopPropagation();
    this.openBillHistoryKey.set("");
  ...
  private consolidatedMaterialBillHistory(group: TableRow[]): MaterialBillHistoryEntry[] {
    const unique = new Map<string, MaterialBillHistoryEntry>();
    for (const row of group) {
      for (const entry of this.materialBillHistory(row)) {
  ...
  private readablePurchaseOrderNumber(value: unknown): string {
    const poNumber = String(value || "").trim();
    return this.isReadablePurchaseOrderNumber(poNumber) ? poNumber : "";
  }
  ...
  async openAddOpeningCash() {
    ...
    const supervisor = allSupervisors.find(sup =>
      sup.name?.trim().toLowerCase() === supervisorName.toLowerCase()
    );

    if (!supervisor || (!supervisor._id && !supervisor.id)) {
      const toast = await this.toastController.create({
    ...
    this.openSupervisorFunding(supervisor._id || supervisor.id, supervisorName, hasOpeningAmount);
  }
  ...
  async submitSupervisorFunding() {
    ...
    if (!supervisorId || !projectId) {
      this.fundingError.set("Missing supervisor or project information.");
      return;
    }
    ...
    try {
      ...
      const siteName = this.fundingSiteId();
      const siteId = siteName ? this.data.resolveSiteNameToId(siteName) : undefined;
      ...
      this.fundingHasOpeningAmount.set(hasOpeningAmount);
      // Keep the in-memory supervisor record in sync so "Add Opening
      // Amount" switches to "Add Cash" without requiring a page refresh.
      this.data.supervisors.update(current => current.map(sup =>
        (sup._id === supervisorId || sup.id === supervisorId)
          ? { ...sup, openingAmountAddedAt: new Date().toISOString() }
          : sup
      ));
      this.fundingAmount.set("");
      this.fundingNote.set("");
      ...
      // Refresh the expenses to show the new transaction, and the sites so
      // a freshly-created opening balance lands in the ledger opening map.
      await this.hydration.loadModule("expenses");
      if (wasOpeningAmount) {
        await this.hydration.loadModule("sites");
      }

      this.closeSupervisorFunding();
      const toast = await this.toastController.create({
        message: wasOpeningAmount
      ...
      let errorMessage = "Could not add funds to the supervisor.";
      ...
      this.fundingError.set(errorMessage);
    }
  }
  ...
  readonly fundingSupervisorHasOpeningAmount = computed(() => {
    const project = this.project();
    const supervisorName = project?.supervisor?.trim();
    if (!supervisorName) return false;
    const supervisor = this.data.supervisors().find(sup =>
      sup.name?.trim().toLowerCase() === supervisorName.toLowerCase()
    );
    return Boolean(supervisor?.openingAmountAddedAt);
  });
